...
async fn cli_compression_handler(
    ...
    // proxy adds [exit: N] to the response regardless of whether compression
    ...
    if let Ok(db) = Database::new(&get_db_path()) {
        ...
        let log_backend = if backend_name.is_empty() { "shell_compression" } else { &backend_name };
...
pub async fn start_proxy_server(app_handle: AppHandle) {
    loop {
        ...
        let db = match Database::new(db_path) {
            Ok(db) => db,
            Err(e) => {
                eprintln!("[PROXY] Failed to initialize database: {}", e);
                let mut status = PROXY_STATUS.lock().unwrap();
                *status = ProxyStatus::Failed(port, format!("Database error: {}", e));
